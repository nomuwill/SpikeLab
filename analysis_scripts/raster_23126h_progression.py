"""
23126h H9SynGFP MO progression — first 5 min per recording.
  D33-D35 : 2026-02-25-e-H9SynGFP_MO_Baseline_haloperidol_dopamine_SCH (ks2SpikeLab)
  D37-D40 : 2026-02-27-e-H9SYNGFP_Rotenone_D0-D3 (NOT YET SORTED — shown as placeholder)
Output: ~/Desktop/Greg/Organized Recording Raster Plots/23126h_progression.png
"""
...
UUID_SCH = "2026-02-25-e-H9SynGFP_MO_Baseline_haloperidol_dopamine_SCH"
UUID_ROT = "2026-02-27-e-H9SYNGFP_Rotenone_D0-D3"

OUT_DIR = "/home/sharf-lab/Desktop/Greg/Organized Recording Raster Plots"
os.makedirs(OUT_DIR, exist_ok=True)

# (label, stem, uuid, source)  source: "pkl" | "none"
RECORDINGS = [
    ("D33 — Control (02/23/2026)",
     "23126h_MO_H9SynGFP_D33_Control_02232026",         UUID_SCH, "pkl"),
    ("D34 — Control (02/24/2026)",
     "23126h_MO_H9SynGFP_D34_Control_02242026",         UUID_SCH, "pkl"),
    ("D35 — Control (New) (02/25/2026)",
     "23126h_MO_H9SynGFP_D35_Control_02252026",         UUID_SCH, "pkl"),
    ("D35 — Control (Connected) (02/25/2026)",
     "23126h_MO_H9SynGFP_D35_Control_connectedconfig_02252026", UUID_SCH, "pkl"),
    ("D37 — Baseline Control (02/27/2026)",
     "23126h_baseline_control_02272026",                UUID_ROT, "pkl"),
    ("D38 — Rotenone 24hr (02/28/2026)",
     "23126h_rotet124hr_02282026",                      UUID_ROT, "pkl"),
    ("D39 — Control 48hr (03/01/2026)",
     "23126h_control_48hr_03012026",                    UUID_ROT, "pkl"),
    ("D40 — Control 72hr (03/02/2026)",
     "23126h_control_72hr_03022026",                    UUID_ROT, "pkl"),
]


def load_from_pkl(stem, uuid):
    tmp = tempfile.mkdtemp()
    try:
        local_pkl = os.path.join(tmp, "spikedata.pkl")
        r = subprocess.run(
            ["aws", "s3", "cp",
             f"s3://braingeneers/ephys/{uuid}/derived/ks2SpikeLab/{stem}/sorted_spikedata_curated.pkl",
...
for label, stem, uuid, source in RECORDINGS:
    print(f"\n{'─'*60}")
    print(f"  [{label}]")
    if source == "none":
        print("  skipping — not yet sorted")
        sds.append(None)
        burst_data.append((None, None))
        stats_list.append(None)
        continue
    try:
        sd = load_from_pkl(stem, uuid)
...
for i, (label, stem, uuid, source) in enumerate(RECORDINGS):
    sd            = sds[i]
...
fig.suptitle(
    "MEA 23126h  —  H9SynGFP MO  —  Progression  —  First 5 min  —  100 ms burst detection",
    fontsize=12, fontweight="bold", y=1.005,
)

save_path = os.path.join(OUT_DIR, "23126h_progression.png")
