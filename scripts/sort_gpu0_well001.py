"""
Sort well001 using Kilosort2 (Docker) pinned to GPU 0.

Patches ContainerClient.__init__ before spikelab imports so that
CUDA_VISIBLE_DEVICES=0 is injected into the Docker container environment.
This means the MATLAB Runtime inside the container only sees GPU 0.
"""
...
# Applied before spikelab imports so spikelab's own __init__ patch wraps ours —
# call chain at sort time: spikelab_patch → this_patch → original __init__.
# Both CUDA_VISIBLE_DEVICES and MW_CUDA_FORWARD_COMPATIBILITY end up in the container env.
...
RESULTS_DIR = (
    "/home/sharf-lab/Desktop/Analysis_shared/data/spikesort_test/"
    "parallel_gpu_test/gpu0_well001"
)
